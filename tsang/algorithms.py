...
def maxmin_algo(grad_oracle, val_oracle, threshold, k, group_indicator, num_iter, num_bin_iter, eps, solver, batch_size=1000):
    S = threshold_include(group_indicator.shape[0], val_oracle, threshold)
    grad_oracle = make_contracted_function(grad_oracle, S)
    val_oracle = make_contracted_function(val_oracle, S)
    ub = 1.
...
#        print(target)
        ...
        vals = val_oracle(x.mean(axis=0), batch_size)
        if vals.min() > target:
            lb = target
        else:
            ub = target
