class TimGraph: public InfGraph
{
    public:
        ...
        void RefindKPT(double epsilon, double ept){
            Timer t(2, "step2" );
            ASSERT(ept > 0);
            int64 R = (2 + epsilon) * ( nodes.size() * log(nodes.size()) ) / ( epsilon * epsilon * ept);
            BuildHypergraphR(R);
        }
        ...
        void NodeSelection(double epsilon, double opt, bool build_SeedSet){
            Timer t(3, "step3");
            ASSERT(opt > 0);
            int64 R = (8+2 * epsilon) * ( log(nodes.size()) + log(2) +  nodes.size() * logcnk(nodes.size(), k) ) / ( epsilon * epsilon * opt); //
            BuildHypergraphR(R);
